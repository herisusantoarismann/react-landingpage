...
import { feedback, IFeedback } from "../constants";
import styles from "../style";
import AnimatedText from "./common/AnimatedText";
import FeedbackCard from "./FeedbackCard";

const Testimonial = () => {
  const [isTextAnimationStart, setIsTextAnimationStart] = React.useState(false);
  ...
  const animationSequence = async () => {
    await headingCtrl.start({ opacity: 1, scale: [1.5, 1] });
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap sm:justify-start justify-center feedback-container relative z-[1]">
        {feedback.map((card: IFeedback, index: number) => (
          <FeedbackCard
            key={card.id}
            {...card}
            cardCtrl={cardCtrl}
